...
- **TrendEngine**: 趋势分析引擎，新增的需求趋势预测模块
...
3. WHEN 重构 QueryEngine 时 THEN FoxTrends SHALL 将其改造为 TrendDiscoveryAgent 用于需求趋势发现
4. WHEN Agent 协作时 THEN FoxTrends SHALL 保留 ForumEngine 的论坛协作机制
5. WHEN Agent 生成输出时 THEN FoxTrends SHALL 复用 BettaFish 的 SummaryNode 和 ReflectionNode 模式
...
**用户故事**: 作为产品经理，我希望系统能够分析需求趋势，以便预测哪些需求方向值得投入资源。

#### 验收标准

1. WHEN 分析需求数据时 THEN TrendEngine SHALL 识别需求的时间序列变化模式
2. WHEN 评估需求热度时 THEN TrendEngine SHALL 基于讨论频率、参与人数、情感倾向等维度计算需求热度
3. WHEN 预测需求趋势时 THEN TrendEngine SHALL 使用统计模型预测需求的未来发展趋势
4. WHEN 对比需求时 THEN TrendEngine SHALL 支持多个需求方向的横向对比分析
5. WHEN 生成趋势报告时 THEN TrendEngine SHALL 输出结构化的趋势分析报告

### 需求 5: Dashboard 前端开发

**用户故事**: 作为系统用户，我希望通过直观的 Dashboard 查看需求分析结果，以便快速了解社区动态和需求趋势。
...
4. WHEN 查看趋势图表时 THEN Dashboard SHALL 使用可视化图表展示需求趋势和对比分析
5. WHEN 导出报告时 THEN Dashboard SHALL 支持生成和下载需求分析报告（复用 ReportEngine）

### 需求 6: ForumEngine 讨论机制扩展
...
3. WHEN 记录趋势数据时 THEN FoxTrends SHALL 创建 trend_analysis 表存储趋势分析结果
4. WHEN 关联数据时 THEN FoxTrends SHALL 建立表间外键关系确保数据完整性
5. WHEN 迁移数据时 THEN FoxTrends SHALL 提供数据库初始化和迁移脚本
